...
- **`telegram.go`** — Optional Telegram long-polling monitor. It only starts when bot token and allowed chat id env vars are present. Supports `/status`, `/usage`, `/metrics`, `/models`, `/help`; unauthorized chats are ignored and Telegram failures never stop the proxy. Telegram messages use HTML parse mode with light emoji, grouped bold headings, bullet rows, and `<code>` for commands/models. Any future Telegram message should follow this format and escape dynamic values with `tgEscape`.
