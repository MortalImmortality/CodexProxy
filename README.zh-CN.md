...
`codex-proxy reset` 默认只查询每个账号可用的 rate-limit reset credit 数量，不会消耗额度。传入一个账号名或邮箱（例如 `codex-proxy reset user@example.com`）时，才会消耗官方 Codex 提供的一个 rate-limit reset credit，并只作用于该账号。reset 成功后，代理会清除该账号本地 failed 标记，让它立刻重新回到账号池可选状态。
...
/reset    手动消耗一次账号 rate-limit reset credit
...
codex-proxy reset                                  # 查询可用 reset credit 数量
codex-proxy reset ACCOUNT_OR_EMAIL                 # 手动 reset 指定账号额度
...
- 某账号触发 429 时会暂时从正常选择池排除，直到上游恢复时间；如所有账号都失败，会回退尝试全部账号
- 可用 `codex-proxy reset` 查询每个账号可用的官方 reset credit 数量；需要消耗时，用 `codex-proxy reset ACCOUNT_OR_EMAIL` 或 Telegram `/reset NAME` 手动 reset，让账号立即重新加入选择池
