...
```bash
codex-proxy usage
codex-proxy usage --reset --account main
...
手动 reset 会消耗官方 Codex 提供的一个 rate-limit reset credit，只作用于指定账号。reset 成功后，代理会清除该账号本地 failed 标记，让它立刻重新回到账号池可选状态。该操作不会自动触发，必须显式执行命令。
...
codex-proxy usage --reset --account NAME           # 手动 reset 指定账号额度
...
- 可用 `codex-proxy usage --reset --account NAME` 或 Telegram `/reset NAME` 手动使用官方 reset credit，让账号立即重新加入选择池
